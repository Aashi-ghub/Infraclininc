...
export default function SoilLabReportForm({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.borehole_no || formData.soil_test_data.length === 0) {
      toast({
        variant: 'destructive',
        title: 'Error',
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <strong>Borehole:</strong> {formData.borehole_no}
                  </div>
                  <div>
                    <strong>Date:</strong> {formData.date ? format(formData.date, 'PPP') : 'Not set'}
                  </div>
                </div>
                
                <div>
                  <strong>Test Samples:</strong> {formData.soil_test_data.length}
                </div>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead>Soil Class</TableHead>
                    ...
                    <TableBody>
                      {formData.soil_test_data.map((sample, index) => (
                        <TableRow key={index}>
                          <TableCell>{sample.sample_no}</TableCell>
                          <TableCell>{sample.sample_depth} m</TableCell>
                          <TableCell>{sample.soil_classification}</TableCell>
                          <TableCell>{sample.moisture_content}%</TableCell>
                          <TableCell>{sample.bulk_density} g/cc</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
